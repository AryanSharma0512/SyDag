...
always returns the list in `src/mock/fieldsData.ts`, in both modes. It describes the sources;
it is not live connectivity. The Data Explorer's "Connected" means the latest
`/api/context/all` response reported `status: ok` for that source.
